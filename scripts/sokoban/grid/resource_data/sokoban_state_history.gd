...
func starts(start: SokobanGridState):
	start_state = start.duplicate(true)
	history.clear()
	redo_history.clear()

func do(new_state: SokobanGridState):
	history.append(new_state.duplicate(true))
	redo_history.clear()

func restart(discard_state: SokobanGridState) -> SokobanGridState:
	redo_history.append(discard_state.duplicate(true))
	return start_state.duplicate(true)

func undo(discard_state: SokobanGridState) -> SokobanGridState:
	if history.size() > 0:
		redo_history.append(discard_state.duplicate(true))
		return history.pop_back().duplicate(true)
...
func redo() -> SokobanGridState:
	if redo_history.size() > 0:
		...
		history.append(redo_state.duplicate(true))
		return redo_state.duplicate(true)
	return null
